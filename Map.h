...
private:
  ...
  // A custom comparator
    class PairComp {
    public:
        bool operator() (Pair_type n, Pair_type n1){
            Key_compare less;
            return less(n.first, n1.first);
        }
  };
...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const{
        Pair_type test = {k, Value_type()};
        return tree.find(test);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k){
        Pair_type test = {k, Value_type()};
        if (tree.find(test) != tree.end()){
            return (*tree.find(test)).second;
            
        }
        else {
            return (*tree.insert(test)).second;
        }
    }
