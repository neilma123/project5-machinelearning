...
private:
    ...
    static int size_impl(const Node *node) {
        ...
        int max = 0;
        max += 1;
        max += size_impl(node->left);
        max += size_impl(node->right);
        return max;
    }
    ...
    static int height_impl(const Node *node) {
        ...
        int len_right = 1;
        if (node->left){
            len_left += height_impl(node->left);
        }
        else if (node->right){
            len_right += height_impl(node->right);
        }
    ...
    static Node * insert_impl(Node *node, const T &item, Compare less) {
        if (find_impl(node, item, less)){
            return find_impl(node, item, less);
        }
    ...
    static bool check_sorting_invariant_impl(const Node *node, Compare less) {
        ...
        if ((node->left) && (node->right)){
            if (less(node->left->datum, node->datum)
                ...
                return (check_sorting_invariant_impl
                        (node->left, less)) &&
    ...
    static void traverse_inorder_impl(const Node *node, std::ostream &os) {
        ...
        //go deepest left then print and return then right
        if(node != nullptr){
    ...
    static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
        ...
        if (less(val, node->datum)){
            auto recurse_left = min_greater_than_impl(node->left, val, less);
            if (!recurse_left){
                return node;
            }
            else {
            ...
            auto recurse_right = min_greater_than_impl(node->right, val, less);
            return recurse_right;
        }
    }
    void print_impl(Node *n) {
        if (!n) { return; }
        print_impl(n->left);
        std::cout << n->datum << " ";
        print_impl(n->right);
